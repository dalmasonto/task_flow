...
Read via `useLiveQuery` (`dexie-react-hooks`); writes debounced (~400ms) so panning does not hammer IndexedDB.
...
**Frontend:** `v2_fe/package.json` (+`dexie` ^4.3.0, +`dexie-react-hooks` ^4.2.0 — both verified to resolve on npm), `src/lib/design-ui-state.ts` (new: Dexie store), `src/lib/design-layout-api.ts` (new: GET/PUT), `src/lib/design-devices.ts` (`layoutBands`, `layoutGroups`, `boardWidth`, gutter 140), `src/pages/design/DesignSurfacePage.tsx` (view picker, hydration gate, Pages-panel group picker), `src/pages/design/design-canvas.tsx` (header containment, dark backdrops), tests alongside the pure helpers.
