...
- **Dead code in `phase3_agent_surface.rs`, pre-existing and confirmed at two commits.** An unused import (`taskflow_agent_credential`) and a `ctx_body(project)` helper with **no callers** — both verified present at `d424027` *and* at `6e86bb2`, so Task 12 did not introduce them even though it edited that file. Worth one line of a later batch: a helper with no callers reads as "used somewhere", and the next person adding a context test may write the URL by hand as Task 12's did rather than reaching for it. **Fix: use it or delete it.** Not urgent, and not this plan's doing.
