...
- **Publish order: the backend must deploy before the frontend.** Measured, not assumed: the deployed backend answers `403` to a realtime group it does not know and the realtime layer refuses the *entire* handshake. Phase 4's held build is still held. So **no task in this plan runs `npm run build`**: a frontend build produced before the user's backend deploy is precisely the hazard this line describes, and Task 9 owns the plan's single one, on the user's go-ahead.
  **And the ordering is simpler than recorded, which is worth knowing before triggering anything** *(corrected 2026-09-25, read from the workflow)*: **`deploy-backend.yml` builds and ships the frontend too.** Its job does `npm ci` in `v2_fe`, sources the decrypted prod env, runs `npm run build` (asserting `dist/index.html` exists), and stages the result as `payload/frontend` alongside the binary — so **triggering the backend workflow deploys both halves atomically, from one commit.** That is *safer* than the two-step sequence this line used to imply: within one workflow the handshake mismatch cannot arise at all. The hazard survives in exactly one path — running **`deploy_frontend.yml` alone against an older backend** — so the rule is not "two steps in order" but **"never ship the SPA without its backend; the backend workflow is what does that."** `deploy_frontend.yml` is then the explicit SPA sync (it builds with `--base /` and also publishes GitHub Pages), and running it after a backend deploy is a redundant-but-harmless second copy of the same code. **The user should be told this before they trigger anything**, because "I only ran the backend one" is not what happens.
...
- [ ] **Step 5: Publish — backend first, and the build comes after it.** Per the measured constraint: the frontend must not go out before the backend, or the realtime handshake 403s app-wide. So this step is ordered, not parallel: **the backend deploy happens first, and `npm run build` — the plan's single build — only after it**, on the user's go-ahead. Their words were "deploy the backend first, then I build".
  ...
  **What the user should actually be told before they trigger anything** *(added 2026-09-25 from the workflow files; see the corrected Global Constraint)*: **`deploy-backend.yml` ships the frontend as well** — it runs `npm ci` and `npm run build` in `v2_fe`, asserts `dist/index.html`, and stages the result as `payload/frontend` next to the binary. So triggering the backend workflow deploys **both halves from one commit**, and does so atomically, which is the safest form this can take. `deploy_frontend.yml` is the SPA's own sync (it builds with `--base /` and also publishes GitHub Pages) and is redundant-but-harmless after a backend deploy. **The only dangerous action is running `deploy_frontend.yml` alone against an older backend**, and it is worth saying so out loud rather than trusting the two-step phrasing to imply it.

  **One behaviour, for the release note rather than for a fix** *(added 2026-09-25, Task 31 item 3)*: a stored `styles/resources.json` that names a `rel` outside the allowlist — `preload` is the one that was removed — is refused by `resources::validate`, and the manifest's forgiving read then contributes **no links from that document at all**, not just the offending entry. Exposure today is nil, and it is worth saying on what: the WRITE path refuses such a document (`validation::validate_resources`, rule `resources`), so neither the editor nor an agent write can create one, and this phase's publish has never run, so no project can already hold one — no fixture, seed or test document anywhere contains `preload` either. It is the trade the allowlist tightening was, not a defect: an entry that fetched nothing while sitting in the DOM was the worse half. Re-adding `preload` means adding `as` to the model first, which is what `resources.rs`'s `ALLOWED_REL` doc says; it is not a one-word revert.
...
- [ ] **4. The order must be visible where it is claimed.** If the panel lists pages in `routeOrder`, say so and make it so; and the canvas boards must follow the same order, or the panel and the canvas will disagree about the flow the user just built. Note the constraint §F records: the canvas must not *reflow* on a grouping edit — but this is an explicit reordering, so re-flowing the boards in the new order is the point. Make that distinction in a comment, because the two look alike.
  ...
  **Outcome, and one ruling owed to it** *(added 2026-09-25 after the task landed)*: the ruling above was followed **and extended** — the panel now lists and numbers in the flow, and `groups` columns order their pages by it. Two consequences were retired deliberately and **ratified**: `numberedPages` had to start receiving the arrangement document (the previous task wrote its not-receiving as an absolute rule, which this ruling necessarily inverts), and the canvas's `g.routes` **assignment order** no longer wins inside a column. **The one thing that needs pinning: `groups` columns previously defaulted to assignment order and now default to the pages' own order, and no existing test pinned that**, so nothing went red for a silent change to what the user sees. **Ruling: pin it with a test**, because a visible behaviour should not rest on "no test objected" — if the review approves without it, a one-test fix round supplies it.
...
- [ ] **7. `composer.rs`: everything the file accumulated while other tasks held it.** *(Retitled 2026-09-25 — this was "the three items Task 22 deferred", but Task 10's re-review added three more to the same file, and grouping by file is what keeps one writer per file.)* **`composer.rs` has had four sequential writers by the time you get here** (Task 10, then Tasks 11 and 12, then this), so **locate everything by symbol**: Task 10's rewriter was substantially rewritten in `b7c57f7` and every line number below predates whatever landed after it.

  **The three items Task 22 deferred under a scope cut.** Task 22 was dispatched while Task 10 was editing the file, so I removed these from its brief to keep one writer per file. Task 22 never touched it and wrote the exact change it *would* have made for each (in `task-22-report.md`), so they are ready to apply. Take them in this order:
  - **7a. The comment that claims a bound the marker does not have — and the patch Task 22 wrote for it is right about the code but under-corrects its own argument.** *(Sharpened 2026-09-25 from Task 22's review, which verified this at the source rather than from the report, because this text is about to be applied verbatim.)*
    **The falseness is confirmed.** `validation.rs`'s `MARKERS` holds `("<script src", false)` — a case-insensitive literal **substring** search whose `false` means *no tag-boundary check*, beside `("<head", true)` and `("<body", true)`, which do check. Nothing else in `validate_page_fragment` looks at a url or a `src` attribute, and the sandbox CSP carries `script-src … https:` — so **`<script type="module" src="https://…">` passes validation and loads**, while two comments assert that "a page fragment cannot name a remote script statically in its markup".
    **What the patch must do beyond narrowing the parenthetical.** One of those two comments offers this as the **first of "two independent blockers"** for why Lottie does not justify widening `connect-src`. Once the parenthetical concedes that one extra attribute defeats the marker, the surrounding sentence still implies a containment that does not exist. So the corrected text must do one of these two things **and say which**:
      - **drop that blocker** and rest on the remaining one (`fetch()` → `connect-src`, which is untouched); or
      - **replace it with what the marker actually is** — a **shell-ownership rule** (a page fragment may not claim the document's own `<script>`/`<head>`/`<body>` slots), whose substring form is a *proxy* for that rule, **not a security bound**.
    At the second site the existing parenthetical already covers the inline-`<script>` route, so the report's proposed text there largely **restates what is already written — trim it rather than pasting it.** **A comment that overstates a safety property is worse than no comment**, and this file has already carried one false rationale past two reviews.
    ⚠️ **There are THREE copies of this claim, not two, and the third is agent-facing.** *(Added 2026-09-25 from Task 12's review.)* A third copy lives in the guidance an agent reads when writing pages (`agent_views.rs`, the `<script src` sentence) — and it is **correct as written there**, because it makes the narrow claim ("page fragments may not contain one") and its remedy ("write a component") is the thing that actually works. So **do not broaden it**: an agent does not need the general form, and telling it more would weaken advice that currently holds. **Decide all three together anyway**, because the reason they diverged is that nobody counted them: two overstate and one does not, and after this task the three should agree about *what the marker is* even though only two need rewriting.
  - **7b/7c.** The second comment narrowing and item 5's runtime-side pointer, per the report's written-out diffs. **When 7c adds the runtime-side pointer, update `design-selection.test.ts:30-34`'s reciprocal note** — it currently says the pointer is one *"which `composer.rs` does not carry"*, which your change makes false. See Task 30 item 15b.

  **Three minors from Task 10's re-review, same file, same family.** Its re-review **approved** the task and reproduced every claim by reverting the fix — including a 40-shape attack battery that found no shape rewritten incorrectly — but the new attribute reader introduced one behaviour change worth closing:
  - **7d. `opens_new_tab` false-positives on an unquoted value containing `target=_blank`.** `<a href="/app" data-x=target=_blank></a>` is read as a new-tab link, so the href is left alone and `/app` 404s on the sandbox origin — **round 1 rewrote it**, so this is a behaviour change from the new reader. Contrived markup, and it errs conservative, hence Minor. **Fix: add `b'='` to the excluded predecessor set** — and **keep** the whitespace and quote cases, or `<a href="x"target="_blank">` would regress the other way.
  - **7e. The new root arm rewrites href-shaped text inside scripts and comments, on the root page only.** `<script>const s = 'href="app"'</script>` on `/` now emits `href="/s/tok/app"` inside the JS string. The **site-absolute** form already had this wart (pre-existing), but for **relative** hrefs it is newly reachable. No impact on a page that does not contain that literal text; note it rather than over-engineering a fix.
  - **7f. `./app` on the root page is still dead, and the "different job" defence does not cover it.** `new URL("./app", "http://h/s/tok")` → `/s/app`, measured identically to `app` — yet the root arm rewrites `app` and leaves `./app` alone. It was equally dead at the diff base, so this task does not make it worse and the implementer disclosed it. But *"resolving dot segments is a different job"* is true for `../` (which genuinely leaves the namespace) and **not** for `./`, which is a one-line strip. Either do the strip or say plainly that `./` is left dead on purpose.
...
- [ ] **15. Four record-and-test corrections from Task 22's review, all one-liners.** Task 22 passed (0 Critical, 1 Important, 6 Minor), and every one of its findings lands **in the record or in a test**, not in the code it shipped:
  - **15a. `task-18-report.md` now under-claims, and this commit is what made it false.** Its §8 item 5 says the `ancestorComponents` → `selection.component` fallback is untested ("the fixture always sends both") — **a test does cover it now** (`design-selection.test.ts:154-179`), so the note sends the next reader to conclude an unguarded path. Meanwhile §5 item 8 still credits "test 8", which **cannot fail for that reason**: its fixture stamps `app-component` at the innermost index, so the `||` never evaluates the fallback. Fix one line in each, with the implementer's words left intact. **A false "untested" note is the same class as a false "tested" one, and this phase has now produced both.**
  - **15b. The reciprocal fixture note is about to be falsified by this very task.** `design-selection.test.ts:30-34` says the runtime pointer is one *"which `composer.rs` does not carry"* — and **item 7 of this task adds it to `composer.rs`**. So the honest note becomes the next stale comment of the class it was written to close. **When you apply item 7, update that sentence to point back.**
  - **15c. `design-inspector.test.ts:281`'s `expect(html).toContain("main")` is near-vacuous** — `main` is also the selection's `tag`, rendered elsewhere in the same markup, so the assertion holds even if the breadcrumb drew nothing. The discriminating assertion in that test is the `not.toContain` beside it (proved by mutation). Swap the vacuous one for the `title="Selected element"` form the sibling test already uses, which is what makes "the chain is on screen either way" actually testable.
  - **15d. The item-1 guard sits at the field boundary, not the value boundary.** `{"accent": null}` still throws (`Cannot read properties of null (reading 'light')`), as does `{"colors": null}`. Calibrated Minor because the document cannot render anyway — `token-editor.tsx` reads `value.light` on the token and crashes first, so a filter-only fix would not make that file usable. The two-line hardening if you take it: `typeof value !== "object" || !value` → no match, and `Object.entries(tokens ?? {})`.

- [ ] **16. The `pages-panel` render test's blind spot is real, empty, and one line to close — and my ruling on it was wrong twice.** *(Corrected 2026-09-25, replacing two earlier and opposite rulings of mine.)* Task 22 asked me to rule on a "test-strength descriptor" that seemed to map to no brief item; I first ruled it was **noise**, then found it in Task 16's parked list and ruled it a **real gap**. Task 22's reviewer settled it by **rendering the actual fixture** (6,936 characters) and checking: the manifest title appears **nowhere** in that markup — not as text, not in any attribute — and there are **zero** `placeholder=` occurrences, because `LabelInput` mounts only from client state that SSR cannot enter, and the add-group input lives in a Base UI portal that renders nothing server-side. **So there is no live bypass**, and there is also a genuine blind spot: `not.toContain(">Settings<")` is anchored on a text node, so the moment `LabelInput` ever renders server-side the same assertion silently stops covering the manifest title.
  **Fix, free and verified to pass today:** assert the absence of the *string* (`expect(html).not.toContain("Settings")`) or the sharper precondition `expect(html.match(/placeholder=/g) ?? []).toHaveLength(0)`. Both fail on any attribute-borne leak. **This is a one-line hardening, not a bug fix** — the correct summary of my two wrong rulings is that I was describing a *shape* rather than a *finding*, which is neither a phantom nor a live defect.

- [ ] **17. Task 25's fix round replaced one unsupported rationale with another, in the very comments it wrote to fix that class** (Task 25's re-review, Minor — and it is the most instructive item in this batch). The new comments say the extracted rule *"had a bug of its own"* (`canvas-paint.ts:13-16`) and is *"the part that was got wrong once"* (`canvas-paint.test.ts:60-62`), echoed in `8b29f02`'s message as *"the one that had already produced a bug (the mid-gesture snap-back)"*. **No committed revision supports any of them**: `107f3b9` already carried `if (!gate.pending() && transform !== liveRef.current) { liveRef.current = transform }`, semantically identical to the extracted `resolveLive`. So the claim rests on uncommitted drafting that nobody can see. The reviewer put it exactly right: **this is the same species as the false IndexedDB claim the round existed to remove — a rationale the code does not support** — and it is unfalsifiable rather than false, which is arguably worse. **Fix: use the original review's own wording, "walked by hand and never pinned".** It is true, it says why the test is worth having, and it needs no provenance.

- [ ] **18. Two wording corrections in Task 25's new module and report.** `canvas-zoom.ts:12-14` says the residual coupling is *"the tests that reach them through it (`canvas-view.test.ts`, `design-ui-state.test.ts`)"* — right for the first, but `design-ui-state.test.ts` never imports `design-canvas` at all; its coupling is **transitive** via `design-ui-state.ts:19`. The disclosure's substance (both still drag React into a pure test) is verified correct by bundling; only the mechanism wording is a step off. And report §6 says the `push` warning *"explicitly exempts"* a monotonic counter — it does not: the warning is **conditional** ("a caller that needs the UNION"), so a counter is simply outside it, and the exemption *and* its explanation live in `content-epoch.ts:33-46` (Task 27's file, predating this fix). Report-only, no code change.

- [ ] **19. The false-claim sweep's arithmetic was off by one in each direction, and it missed a copy** (Task 25's re-review). It reported four code sites false; **one was true as written** (`settle-gate.test.ts:69-70` — the write really does sit behind the committed state, so a gate that never fires really does leave it behind), so "had spread to four places" **overstates by one**. And it counted **two** untouched copies (history, the plan) when there is a **third**: the generated brief file `.superpowers/sdd/…/task-25-brief.md` carried the sentence verbatim, because the dispatcher copies plan text into briefs. **The general lesson, worth stating once here rather than per task: a generated brief inherits every defect in the plan verbatim, so correcting the plan does not correct the brief** — regenerate it if the task is still pending, and do not count it as a separate site if it is not.

- [ ] **20. A page deleted in another tab can make every later layout save fail — `routeOrder` is fixed, `groups` and `pageLabels` are not.** *(Filed 2026-09-25 from Task 26's own concern 2, which disclosed it as a residual rather than as a finding. It is a finding.)*
  **What Task 26 already fixed:** `updateLayout` now filters `routeOrder` against this project's manifest, because the document is PUT **whole** and the server's strict write path returns **400** for an unknown route — so before that filter, a page deleted elsewhere made **every subsequent layout save fail, a rename included**, since this tab fetches the manifest once per project.
  **What is still broken:** the same strict 400 applies to a stale entry in **`groups[].routes` or `pageLabels`**, and that fix does not reach them. **Silent filtering is not the fix here** — dropping a route from a group or a label silently would discard a user's decision instead of repairing it, which is the opposite mistake. **The correct fix is a manifest refetch when the design-file event says the manifest changed**, so the client's copy is current rather than pruned.
  **Ruling (2026-09-25): this item is NOT part of this batch — it becomes its own task.** Task 30 has grown from 7 items to 22 as reviews parked findings, and 21 of them are text corrections, one-line guards and comment repairs that share a shape: low risk, no new mechanism, verifiable by a passing suite. **This one is not that shape.** It needs a new mechanism (refetch the manifest when the design-file event says it changed), it lives in the realtime handler that every surface depends on, and its failure mode is a save that silently stops working. Mixing it into a sweep of wording fixes is how a behaviour fix gets done badly or quietly deferred — and "say which you chose" is exactly the out that lets it be deferred forever. **Do it as its own dispatch, immediately after Task 30**, so the two do not share a diff. If it turns out small, that costs one dispatch; if it is not small, the batch is not held hostage by it. **Why it is worth filing at all:** the failing action is ordinary — rename a page in one tab, then rename another in a second tab, and the save fails with a validation error about a route that no longer exists. Nothing in the UI explains that, and the user has two tabs open whenever they are comparing pages.

- [ ] **21. `PagePicker` is now the only page listing that does not follow the flow** (Task 26's review, Minor). The trigger reads `openRoutes[0]` and the menu walks `routes` in manifest order — so Task 26's report flagged the trigger, but **the menu is the same omission**, and its claim that the picker "makes no claim about the flow" is true of the menu and not of the label. It is acceptable as a control rather than a presentation listing, and the fix is either to order the menu by the flow (it already receives `nameFor`; it would also need the resolver) or to add one comment stating it is **deliberately** manifest-ordered so the next reader does not file it as a bug. **Take the comment unless ordering it is trivial** — a control that is deliberately not the presentation order is fine; a control nobody decided about is not.

- [ ] **22. Two nits from Task 26's re-review, both in text a reader will trust** (Minor). (a) `DesignSurfacePage.tsx:319`'s invariant note says *"the invariant every other writer here maintains deliberately"* and then enumerates the four writers that **establish** manifest order — but two other writers also exist, the close paths at `:460` and `:467`, which are `current.filter(…)` and **preserve whatever order they are handed**. They cannot *introduce* a divergent order, so the substance is right and only the word is too broad. Say *"every writer here that **establishes** the order"*, or add that the close paths only filter and carry the order through. **Worth fixing rather than tolerating** because this is the note a future author reads before deciding whether they may reorder `openRoutes`, and "every other writer" reads as a complete inventory. (b) The pin's `open` list in `design-devices.test.ts` is `["/", "/login", "/settings"]` — **already sorted**, so it discriminates `openRoutes` order from `g.routes` assignment order (its actual point) but would not notice a future refactor making the empty-flow fallback *be* a sort. That blind spot is inherited repo-wide convention (`PAGES` is sorted too), so it is not introduced here; **a non-alphabetical `open` closes it for free** if you are in the file.

- [ ] **23. An agent-facing validation message tells agents to put fonts where fonts cannot go** (Task 12's review, Minor, pre-existing and parked nowhere until now). `validation.rs`'s refusal message says *"Fonts and other assets ship as local files under `assets/`."* — but **`assets/` admits image extensions only** (`.svg/.png/.jpg/.jpeg/.webp/.gif/.ico`), and since the resources task the intended home for a font is `styles/resources.json`. So an agent that trips this message is told to do the one thing that will also fail, and it will loop. **This is the inverse of the task that just landed** — that task existed to tell agents what they *can* do, and this message tells them something they cannot. **Fix: correct the message text.** Also decide whether the same wording appears elsewhere (grep the message's phrasing, not just the file) — a validation string copied into two places is the same defect twice.

- [ ] **24. Verify and commit.** `cd v2_fe && npx tsc -b && npm test && npx eslint <touched files>` — baseline **27 errors / 1 warning**, measured **per file** (a repo-wide read can be inflated mid-round by a neighbour's untracked file — that has now produced three wrong counts in this phase, so measure the committed tree or per file). **Do not run `npm run build`**; no push. Commit with `git commit -F <msg> -- <paths>`.
...
- [ ] **7. The render test's `not.toContain` blind spot — settled, and it is a free one-liner.** *(This item has been wrong twice in both directions; Task 22's reviewer settled it by rendering the actual fixture and checking, which is the only thing that could have.)* **No live bypass exists**: the manifest title appears nowhere in that markup — not as text, not in any attribute — and there are **zero** `placeholder=` occurrences, because `LabelInput` mounts only from client state SSR cannot enter. **The blind spot is nonetheless real and empty by construction**, so the moment `LabelInput` ever renders server-side the text-anchored assertion silently stops covering the manifest title. **The fix is already written as Task 30 item 16** — assert the absence of the *string*, or assert zero `placeholder=` occurrences — so this item is a **pointer, not work**. My two opposite rulings (first "noise", then "a real gap") were both descriptions of a *shape* mistaken for a *finding*.
...
- `screenshots.rs:112`'s `unused_mut` — the **only** warning in a clean `cargo build --release --locked` of the whole workspace (run 2026-09-25, exit 0, 3m04s). Pre-existing, in a file no task in this plan touches. **One word to remove if you happen to be in that file**; otherwise leave it, because a release build's warning count is not worth a separate dispatch.
- **Dead code in `phase3_agent_surface.rs`, pre-existing and confirmed at two commits.** An unused import (`taskflow_agent_credential`) and a `ctx_body(project)` helper with **no callers** — both verified present at `d424027` *and* at `6e86bb2`, so Task 12 did not introduce them even though it edited that file. **Resolved: delete both, do not use them.** *(Corrected 2026-09-25 — my first version of this line said "use it or delete it", and Task 12's review showed the choice is not open: `ctx_body` returns a request **body** (`json!({"project": …})`) while the context read is a **GET** whose project travels in the query string, so the shape cannot fit any caller, which is presumably why it was orphaned.* The test that "should" have used it follows the file's own convention instead — the pre-existing `agent_reads_context_and_registry` builds the identical URL inline.) **The lesson worth keeping: "use it or delete it" was a false choice I offered because I read the helper's name and not its signature.**
...
- [ ] **14. One marker is missing from the agent-guidance test, and it is the claim itself** (Task 12's re-review). The test's 18 markers were **proved non-tautological** — the re-reviewer ran all 18 against the pre-fix text and found the only five absent were the five newly added, each a distinctive single-occurrence phrase — so this is a narrow gap, not a weak test. **The gap: nothing pins the operative words of the Back claim.** The markers cover the *setup* (`opened directly at one route`) and the *remedy* (`A link to a known route always`), so a rewrite to "Back there works" would still pass while the sentence said the opposite of what it was added for. **Fix: one marker for the claim itself** (the reviewer's phrasing: "and Back there does nothing"). **Worth doing rather than filing** exactly because the surrounding markers are good: a test whose every neighbour discriminates, except the one guarding the new claim, is the shape that survives review.

- [ ] **15. A report sentence that the fix round falsified** (Task 12's re-review, Minor). `task-12-report.md:164` still says, in the **present tense**, that "the served text is byte-identical to the brief's two blocks" — but fix round 1 added two clauses (one I drafted, one the implementer wrote) that are **not** in the brief. The `1552`-char and `13`-marker numbers *were* correctly labelled "at `6e86bb2`"; this one was not, and it is now the only pre-fix claim left standing unanchored. One clause fixes it. **The class matters more than the instance:** the implementer was careful about exactly this and still missed one, which is the argument for anchoring numbers as you write them rather than in a later sweep.

- [ ] **16. A comment in `composer.rs` states the unqualified version of the claim the guidance now qualifies** (Task 12's re-review, Minor, and a natural neighbour of item 5). The doc comment asserts *"back/forward — and an agent-written `history.back()` — work with nothing further built."* It is **true of the after-a-click case**, so it is not false — but it is the same sentence the agent-facing guide now qualifies (a frame loaded directly at a route has one history entry, so `back()` is inert), and **two documents disagreeing about the same behaviour because one was written before the other learned better is this phase's most-repeated defect class**. Fix: one line, making the comment say what the guide says. **Take it with item 5** — same file, same class, same one-line shape, and `composer.rs` has had four sequential writers so a single visit is worth more than two.

- [ ] **17. Soften one absolute — the route stamp's "same document" claim is falsifiable in a one-second window** (Task 11's re-review, Minor). *(The re-reviewer proved the fix first: it drove **headless Chrome with real React 19 and the actual `design-route.ts` on both sides**, and the pre-fix structure failed both reachable paths — close-then-reopen and project switch with a new token — while the fix passed both. This is a residue of that design, not a hole in it.)*
  **`design-route.ts`'s stamp doc says "an equal stamp means the SAME DOCUMENT is still mounted". That is true of the iframe's key, but not of the document in one narrow case.** The sandbox token is `HMAC(secret, "{project}.{expiry}")` with `expiry = now + TTL` (`sandbox.rs:36-38`) — **a pure function of project and wall-clock second** — so an A→B→A project round trip **inside one second re-mints A's original token byte-for-byte**. The frame *is* remounted on that path (its `frameSrc` changed twice), the surviving `ArtboardCard` keeps its report, and the stamp matches again: the chip reads `→ /settings` for a frame that was remounted to its home route. Reproduced in the harness.
  **Ruling: soften the absolute; do NOT make the token monotonic.** A monotonic generation would close it, but the token is a **security-adjacent** value and changing its minting to fix a case reachable only by a same-second A→B→A on a board that had already wandered — and which **self-heals** when the new frame announces on `pageshow` — is a bad trade. Say what the stamp actually guarantees (same key, same token, same epoch ⇒ same mounted document *as far as this app can distinguish*), and note the residue so the next reader is not surprised. **Also record the second, smaller residue of the same design if it fits the same comment:** a report already in the event queue when the token flips is stamped with the new token by the re-registered listener. Both are strictly narrower than the pre-fix behaviour, where a stale report survived every remount.

- [ ] **18. A report overstates its own work — the third instance today, and the mildest** (Task 11's re-review, Minor). The fix round's report §0.5 claims the false *"a clear would miss a frame that fails to load"* phrasing **"had also leaked into the code comments"**. `git grep` over **both** revisions for that vocabulary finds **no such claim in any file**. What the commit actually rewrote were two **weak-but-true** comments carrying the epoch-only rationale, and both are now accurate — so **the code outcome is right and the claim about the code is unsupported**. Fix the sentence, leaving the implementer's words otherwise intact. **The pattern is worth one line in this batch's own record:** of the three reviews that landed today, one found a report under-claiming, one found a report whose numbers were another task's, and this one finds a report over-claiming about its own thoroughness — all three are the same defect with different signs, and none of them was in the code.

- [ ] **19. The two reworded agent-facing validation messages are unpinned** (Task 30's review, Minor — the gap is in the record, not the item). `validation.rs`'s two messages were corrected so they no longer send agents to put fonts under `assets/`. **Every test asserts the `rule` (`remote-import`, `remote-url`), never the text** — so nothing fails if the wrong advice comes back, and this message is read by an agent at exactly the moment it has already been refused once. **Fix: one assertion on the text**, or say in a comment that the wording is deliberately unpinned. **Why this one is worth a line rather than a shrug:** the message it replaced had sent agents in a circle for as long as it existed, and the only thing preventing a return is that nobody edits that string. A test is what makes that structural.

- [ ] **20. A comment that reads as if this batch added tests it did not** (Task 30's review, Minor). `canvas-paint.ts`'s comment says the rule was *"walked by hand and never pinned"* and that *"both halves of it are tests now"* — true, but **the two tests predate this commit**; the diff edited only the comment. So the wording implies credit this batch did not earn, and the next reader looking for `git blame` on those tests will find a different commit. **Fix: one clause of tense** — they were already tests, and this batch corrected the comment that denied it. **The instruction to write it this way came from my brief, applied faithfully** — recorded so the implementer is not credited with the imprecision.

- [ ] **21. The drift table mixes two frames under one header, and one row contradicts the report's own body** (Task 30's re-review, found on rows that fix round never touched). The §2 table's header says *"Measured against `7e3a55f`"*, and the three rows the fix round corrected **were** re-pointed — but three untouched rows are pre-batch `21c0839` measurements wearing that header. Worse, one of them is a claim that **nothing moved**:
  - `task-30-report.md:67` says `App.tsx`'s `:483`/`:482`/`:489` are **"unchanged … the only citation in the brief that still lined up"**. At `7e3a55f` that is **false**: the read is still `:482`, but the write is `:497` and the check is `:488` — **because this task's own item 8 is what moved them** — and the report's **§1 item-8 row already says the correct range** (`:490-497`). So the report contradicts itself, and the false half is in the table whose whole purpose is recording what moved.
  - `:62` — `readUIState` cited at `:314`; it is `:319` (the batch moved the effect down five lines).
  - `:69` — `content-epoch.test.ts` cited as `":137-147"`; item 12 **renamed that test**, and it now sits at `:144`.
  **Fix: re-measure the three rows against a commit and say which commit.** **Why it is worth doing rather than deleting the table:** the phase's whole record now leans on it — three separate tasks' implementers reported drift, and this table is what the next person checks against. A table that is right about the rows someone re-checked and silently stale about the rest is more dangerous than one with no header claim at all, because the header is what makes it trustworthy.

- [ ] **22. Verify and commit.** Per file touched, not repo-wide: `cargo test --workspace` for the backend (**`--workspace` is mandatory — a bare `cargo test` in `backend/` silently omits every plugin crate**), and `npx tsc -b && npm test && npx eslint <files>` for `v2_fe`. Baseline **27 errors / 1 warning**, measured per file. **No build, no push** unless the user has lifted the hold by then. Commit with `git commit -F <msg> -- <paths>`; `git add` exact paths first if you create a file.
...
- **The section order, confirmed by the user rather than inferred: Groups first, then the bulk control, then ungrouped pages LAST.** Their words: *"yes, ungrouped in their own section is fine, let ungrouped be the last section."* The panel's own header comment calls its parts "sections", so keep that word and make the order explicit in it — Groups (each with its pages) → Select all/Deselect → **Ungrouped, last**. The bulk control is a control rather than a heading, so it stays between the two lists exactly where it is today; that also keeps it immediately above the only rows that are not inside a group, which is where it reads as "every page below and above".
...
5. **The serialised-key test, and the shape it must not omit.** One test now pins the key set of every shape the frontend mirrors by hand, and mutating `TokenGroup`'s `rename_all` kills it — **before it existed, the `variables_dark` misspelling was invisible to the entire workspace.** ⚠️ **Its coverage claim is the place to be suspicious**: an earlier version claimed *"every shape"* while omitting four, **including `DesignComment` — the 17-key shape whose `pagePath` / `page_path` mismatch is the instance that started this whole class**. If you extend or trust that test, check `design-api.ts`'s own shape list against its assertions rather than against its doc comment.
   **And when that check was actually run, it landed on the instruction itself:** the fix round added `DesignComment` and `DesignFile` and now names its shapes one by one *including the deliberately absent ones* — but it **still omits `TokensDoc`** (named at `design-api.ts:42`, hand-mirrored as `DesignTokensDoc` at `:95`) from **both** the covered list and the named-absent list. So the shape-by-shape list still cannot be read as exhaustive against the enumeration it cites — **the same defect this fact was written to remove, one shape smaller.** Not a live bug (its keys round-trip, so a rename fails the parse loudly), and left as a Minor on the final wave rather than a third fix round. **The instruction above is the part that keeps working: it found the omission only when someone followed it.**
...
7. **`cargo test` without `--no-fail-fast` hides a second killer.** A claim that a mutation killed *one* test was wrong because it killed two, and the second was invisible: **the default stops at the first failing target.** The general form is worth keeping, because the same class produced four wrong numbers earlier the same day: **a tool's default output shape silently narrows what you can observe**, and the narrowed observation then gets reported as a property of the code. If you are measuring what a mutation kills, use `--no-fail-fast`; if you are measuring anything at all against a shared tree, measure from a commit.

8. **What is untracked and therefore not durable.** The 28 task reports, every review package, and this plan's workspace ledger. The commit messages name their commits and the code comments carry the reasons, so the *decisions* survive; the *evidence* — mutation tables, reverted probes, worktree measurements — does not. **If a future phase wants it, the smallest correct change is two lines in `.superpowers/sdd/.gitignore`: `!*/` then `!*/task-*-report.md`** — the bare re-include does not work, because git will not re-include a file whose parent directory is excluded.

9. **"Resume the agents by id" is not a recovery mechanism.** A handoff written before a session restart told the next session to resume two implementers with `SendMessage(to: "<agentId>")`, calling them resumable. They are not: **subagent transcripts do not survive the session** — the call returns *"No transcript found for agent ID"*. What survives a crash is the **work in the tree** and the **ledger**, and nothing else. So a restart handoff must name *files and findings*, never agent ids. **This is the third restart this phase paid for** (facts 5–7 and the two earlier recoveries), and the cost each time is re-deriving context — which here was partly repaid, because making a fresh implementer *audit* the dead draft instead of adopting it found three wrong mutation counts in the very report that recorded them.

10. **A lost verdict is the residue a crash leaves that nobody notices, and the handoff missed one.** An implementer that dies leaves modified files; a reviewer that dies leaves **nothing**. The restart handoff correctly listed Task 31's re-review among the work to redo, and **omitted Task 28's entirely** — whose fix was already committed (`23275a1`), whose scoped re-review had been dispatched, and for which **no line anywhere recorded a verdict**. It would have shipped unreviewed. The general form: **after a crash, enumerate the *review* loops, not just the dirty files** — `git status` cannot show you a missing verdict. Both recovered verdicts came back clean, and Task 28's replacement re-ran the three mutations that had previously left the full suite green, each of which now fails — so the coverage gap really was closed, which is precisely what nobody could have known without the verdict.

11. **A ledger is not a status board, and a keyword audit of one invites the phase's most expensive failure.** Auditing this ledger with `grep -c 'complete (commits'` returned **5** for a phase with 31 finished tasks, and a per-task keyword sweep reported nine tasks as missing — which would have re-dispatched a large part of a completed phase. The lines disagree because `Task N: **COMPLETE**` means *"the implementer finished its first pass"* on some lines (Task 31's, three lines **before** its own review was dispatched) and *"the review closed"* on others. **The reliable question is what a task's *last* line records**, and where a ledger keeps periodic `STATUS BLOCK` corrections, the last one supersedes the per-task lines above it.

12. **Correcting a reason where it was *used* is not correcting it where it was *found*.** The reason that a cold chat send "creates a duplicate of the room that already exists" was disproved by the backend (`find_project_room`, `views.rs:931-983`, landed `1f25924` — an ancestor of the base commit) and corrected in the three docstrings and the `App.tsx` comment that carried it. **It still stands in this plan's own scratch** — `boot-load-map.md` item 3, `task-28-brief.md:20`, and `progress.md:481` ("The duplicate-creation hazard in item 3 is confirmed, not merely suspected"). Nothing shipped, and the workspace dies with the phase — **but the next task briefed from that map would have inherited a false premise**, which is exactly how the reason reached three docstrings in the first place. The correction lives here instead, which is why this section exists.

13. **A stray untracked file is not neutral; it corrupts measurement.** One measurement harness left untracked by a killed session (`v2_fe/src/lib/zz-task29-bench.test.ts`) made `npx tsc -b` **fail repo-wide** (exit 2, all three errors in that file), added **+1 error to both eslint baselines**, and added **+1 file / +1 test** to every count — on top of the three wrong repo-wide counts a stray already produced earlier in this phase. **Every subsequent measurement in the tree was taken against a baseline that file had moved.** The rule is narrower than "delete strays": **a file that is not part of the deliverable must not be in the tree while anything is being measured**, because its cost is not its own content but every number measured beside it.
...
- **Naming the agent guidance in a tool description — cannot land in this phase.** *(Added 2026-09-25 from Task 12's review.)* The `guide` field rides the read agents are told they must always make before their first design write, and it passes through unfiltered into the tool result, so the guidance **is** reachable today. What is missing is one clause in `design_write_page`'s description naming it — and two things block it: the MCP is **served from its built `dist/`, so a source edit is not served until a rebuild**, and the running MCP is a **global copy** (`~/.nvm/.../taskflow-mcp/dist/`), so a repo-side `npm run build` is **also** not enough — a reinstall or relink is required. Task 12's review proved the consequence empirically rather than arguing it: the installed `dist/server.js` is stale and contains **zero** occurrences of `primitives`, which means an earlier pointer written into `mcp/src` **never reached a single agent**. **So this is a post-publish follow-up, and it is the concrete example of a general rule worth keeping: text written into a package that is consumed from a build is not a deliverable until you have read it back out of the running artifact.**
