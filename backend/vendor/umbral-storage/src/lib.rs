                ...
                media_router = media_router.layer(axum::middleware::from_fn(
                    move |req: axum::extract::Request, next: axum::middleware::Next| {
                        ...
                        async move {
                            ...
                            // then PERCENT-DECODED. [VENDORED FIX — mirrors the
                            // umbra checkout; remove this vendor copy at 0.0.11.]
                            // The callback must see the same string `FileField`
                            // stores and `ServeDir` resolves on disk; the raw path
                            // (`Screenshot%20from%20x.png`) makes every spaced key
                            // unmatchable in an ownership lookup. Invalid UTF-8
                            // after decoding keeps the raw string: it matches no
                            // stored key, so the gate fails CLOSED.
