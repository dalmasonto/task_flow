    ...
    async fn object_scope_hook(
        ...
        match hook(identity.cloned()).await {
            ...
            crate::resource::ScopeDecision::RestrictIn(col, values) => {
                // Empty membership → NO rows, never all rows.
                if values.is_empty() {
                    return ObjectScopeOutcome::DenyAll;
                }
                // [VENDORED FIX — mirrors the umbra checkout; remove at
                // 0.0.11.] Bind in the COLUMN's type: the hook speaks strings,
                // but text against a bigint column is a hard Postgres error
                // (`operator does not exist: bigint = text`) — every scoped
                // LIST 500s there while SQLite silently coerces. Integer
                // columns bind parsed i64s (unparseable values can never
                // match and are dropped; all-dropped stays DENY), text and
                // unknown columns keep the strings unchanged.
                let col_ty = umbral::migrate::model_meta_for_table(table).and_then(|m| {
                    m.fields.iter().find(|c| c.name == col).map(|c| c.ty)
                });
                let integer_family = matches!(
                    col_ty,
                    Some(
                        umbral::orm::SqlType::BigInt
                            | umbral::orm::SqlType::Integer
                            | umbral::orm::SqlType::SmallInt
                            | umbral::orm::SqlType::ForeignKey
                    )
                );
                let expr = sea_query::Expr::col(sea_query::Alias::new(col));
                let cond = if integer_family {
                    let ints: Vec<i64> =
                        values.iter().filter_map(|v| v.trim().parse().ok()).collect();
                    if ints.is_empty() {
                        return ObjectScopeOutcome::DenyAll;
                    }
                    sea_query::Condition::all().add(expr.is_in(ints))
                } else {
                    sea_query::Condition::all().add(expr.is_in(values))
                };
                ObjectScopeOutcome::Filter(cond)
            }
        }
