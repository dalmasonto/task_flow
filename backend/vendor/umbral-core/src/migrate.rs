...
/// [VENDORED FIX] Length-aware Postgres type: `Text + max_length > 0` is
/// `character varying(n)`, matching the CreateTable mapping.
...
fn render_alter_column_postgres(
    ...
    // [VENDORED FIX — mirrors the umbra checkout; remove at 0.0.11.] A
    // same-type Text column whose `max_length` changed is ALSO a type change
    // on Postgres — varchar(n) carries the bound in the type — and rendering
    // nothing here is how widened models silently kept their old bound on
